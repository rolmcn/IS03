...
from eigva_app.schemas.auth_schemas import RegistrationData, LoginData
from eigva_app.utils.login_manager import generate_unique_login
from eigva_app.utils.blind_index import generate_login_index
from eigva_app.utils.crypto import encrypt_data, decrypt_data, hash_password, verify_password
from eigva_app.utils.recaptcha import verify_recaptcha
from eigva_app.utils.rate_limiter import check_post_limit, register_post_attempt, reset_attempts
from eigva_app.utils.token import generate_confirmation_token, verify_confirmation_token
from eigva_app.utils.mail import send_registration_confirmation_email, send_registration_success_email
from eigva_app.utils.logger import logger
...
async def login_post(request: Request, session: AsyncSession = Depends(get_async_session)):
    ...
    if "submit_login" in form:
        ...
        response.set_cookie(
            ...
            value=session_id,
            httponly=True,
            secure=False, # <-- gamybinėje pakeisti į True
            samesite="lax",
        )
        return response
